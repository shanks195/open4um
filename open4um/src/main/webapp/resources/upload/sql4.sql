...
SELECT sourceid, sourcename, avatar, ctgname FROM source 
			...
			WHERE ctgid = 28
			SELECT sourcename FROM source WHERE ctgid = 28



SELECT a.ctgname ,b.ctgname FROM category as a LEFT JOIN category as b ON a.ctgid = b.parentid WHERE a.parentid = 33 OR a.parentid = 34
...
SELECT sourceid, sourcename, avatar, ctgname FROM source 
 ...
 SELECT sourceid, sourcename, avatar, ctgname FROM source 
			JOIN category
			ON source.ctgid = category.ctgid
			WHERE category.parentid = 33 AND ctgname="Source Web"
			
			
			SELECT sourceid, sourcename, avatar, ctgname FROM source 
			INNER JOIN category
			ON source.ctgid = category.ctgid LIMIT 0, 3